...
* 计划任务类型 
    ...
    * 软件专用计划任务
        ...
        * 语法:同系统计划任务
        * 示例:同系统计划任务
        * 建议:`/etc/cron.d/`文件夹下的文件应为某个软件或事件专用的文件,不同的软件或事件应新建不同的计划任务文件
* scp下载文件
    * `scp -r [-P PORT] REMOTE_HOST:FILE_PATH LOCAL_SAVE_PATH`
        * 解析
            ...
            * LOCAL_SAVE_PATH:下载到本机的存储路径
        * 示例
            * `scp -r 200_45:/var/www/IR/dev/config/test.php d:\`。本机`.ssh/config`文件中配置了名为`200_45`的`ssh远程主机`,并将文件`/var/www/IR/dev/config/test.php`下载到本地`d:\`路径下
...
* scp上传文件
    * `scp -r [-P PORT] LOCAL_SAVE_PATH REMOTE_HOST:FILE_PATH`
        * 解析
            * PORT:ssh端口。如果端口为22或如果在`.ssh/config`中配置过端口,亦可省略
            * REMOTE_HOST:远程主机 
            * FILE_PATH:远程主机存储上传文件的路径 
            * LOCAL_SAVE_PATH:要上传的文件的本机存储路径 
        * 示例
            * `scp -r d:\test.php 200_45:/var/www/IR/dev/config/`。本机`.ssh/config`文件中配置了名为`200_45`的`ssh远程主机`,并将本地文件`d:\test.php`上传到远程主机`/var/www/IR/dev/config/`路径下 
