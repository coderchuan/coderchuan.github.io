...
* 前提
    ...
    * 安装`php-mbstring`扩展 
* 创建基础Yii项目
    1. 执行命令`composer create-project --prefer-dist yiisoft/yii2-app-basic DIR_NAME`，其中`DIR_NAME`为文件夹名，将会在当前目录下新建一个名为`DIR_NAME`的文件夹，其中包含着YII项目文件。在项目中定位名称空间时，通常把`DIR_NAME`写为`@app\`以表示项目根目录,`app/`表示当前模块根目录,`app\`表示名称空间根目录。
